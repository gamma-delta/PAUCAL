...
abstract public class PaucalRecipeProvider extends RecipeProvider {
  ...
  protected PaucalRecipeProvider(PackOutput out, CompletableFuture<HolderLookup.Provider> regi, String modid) {
    super(out, regi);
    this.modid = modid;
  }
  ...
  protected ShapedRecipeBuilder ringCornered(RecipeCategory category,
      ItemLike out, int count, @Nullable Ingredient cardinal,
  ...
  protected ResourceLocation modLoc(String path) {
    return ResourceLocation.tryBuild(this.modid, path);
  }
  ...
  protected static Criterion<InventoryChangeTrigger.TriggerInstance> hasItem(MinMaxBounds.Ints p_176521_, ItemLike p_176522_) {
  ...
  protected static Criterion<InventoryChangeTrigger.TriggerInstance> paucalInventoryTrigger(ItemPredicate... $$0) {
    return InventoryChangeTrigger.TriggerInstance.hasItems($$0);
  }
}
